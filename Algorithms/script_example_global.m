% A script that applies the global method ([GU2026b], Alg. 2) to a simple
% example. 
...
algo_options.local_flag = 0; % Set to 1 for applying local superlinear solver

% Parameters for global phase
global_options.delta_arr = 10.^(1:-1:-4);
...
% global_options.sp_solver_optns = optimoptions("fmincon","Display","none","Algorithm","interior-point","MaxFunctionEvaluations",10^6);

algo_options.global_options = global_options;

% Parameters for local phase (only relevant if local_flag = 1)
...
if(local_success_flag)
    ...
    h3 = plot(size(x_arr_all,2) + (1:size(result_local_phase(end).x_arr,2)), log10(vecnorm(result_local_phase(end).x_arr - x_min,2,1)),'ko-','MarkerSize',6,'LineWidth',lw);
    plot(size(x_arr_all,2) + (1:size(result_local_phase(end).x_arr,2)-1), log10(result_local_phase(end).eps_arr), 'r:','LineWidth',lw)
end
...
if(local_success_flag)
    xlim([0.5,size(x_arr_all,2) + size(result_local_phase(end).x_arr,2) + 0.5])
else
