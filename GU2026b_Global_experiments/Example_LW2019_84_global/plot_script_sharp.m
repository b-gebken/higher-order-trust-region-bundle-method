% Script that performs the experiment in Example 5.1 (with n = 50, m = 100)
...
global_options.norm_flag = Inf;
global_options.sp_solver = 'linprog';
global_options.sp_solver_optns = optimoptions("linprog","Display","none","OptimalityTolerance",10^-10);
...
legend([h1,h4,h2],{'$\| x^{j,i} - x^* \|$', '$\| x^j - x^* \|$', '$\Delta_j$'},'Interpreter','latex','FontSize',21,'Location','sw');
...
%%
if(save_figure_flag)
    export_fig 'plot_sharp' '-png' '-r500' '-transparent'
end